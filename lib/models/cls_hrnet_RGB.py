...
class HalfDilatedBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None):
        ...
        self.bn1 = nn.BatchNorm2d(planes, momentum=BN_MOMENTUM)
        self.dil_conv1 = nn.Conv2d(inplanes, planes, kernel_size=3, stride=stride, padding=8, bias=False, dilation=8)
        self.dil_bn1 = nn.BatchNorm2d(planes, momentum=BN_MOMENTUM)
        self.conv2 = nn.Conv2d(inplanes, planes, kernel_size=3, stride=stride, padding=1, bias=False, dilation=1)
        self.bn2 = nn.BatchNorm2d(planes, momentum=BN_MOMENTUM)
        self.dil_conv2 = nn.Conv2d(inplanes, planes, kernel_size=3, stride=stride, padding=8, bias=False, dilation=8)
        self.dil_bn2 = nn.BatchNorm2d(planes, momentum=BN_MOMENTUM)
        self.relu = nn.ReLU(inplace=True)
        self.downsample = downsample
        self.stride = stride
...
class HighResolutionNet(nn.Module):
    ...
    def __init__(self, cfg, **kwargs):
        ...
        # self.RGB_layer0_0_dil = nn.Sequential(
        #     nn.Conv2d(in_channels=41,
        #               out_channels=64,
        #               kernel_size=3,
        #               stride=1,
        #               dilation=8,
        ...
        # )
        # self.RGB_layer1 = self._make_layer(HalfDilatedBlock, inplanes=64, planes=32, blocks=1, stride=1)
        # self.RGB_layer1_tail = nn.Sequential(
        #     nn.Conv2d(in_channels=64, out_channels=4, kernel_size=1, stride=1, padding=0, bias=False),
        #     nn.BatchNorm2d(4, momentum=BN_MOMENTUM),
        #     nn.ReLU(inplace=True)
        # )
        self.RGB_dct = dct2d_Conv_Layer(scale=8, start=0, num_filters=192 )
        self.RGB_layer2 = self._make_layer(BasicBlock, inplanes=192, planes=96, blocks=4, stride=1)

        self.RGB_stage3_cfg = extra['STAGE3']
        num_channels = self.RGB_stage3_cfg['NUM_CHANNELS']
        block = blocks_dict[self.RGB_stage3_cfg['BLOCK']]
        num_channels = [
            num_channels[i] * block.expansion for i in range(len(num_channels))]
        self.dc_transition2 = self._make_transition_layer(
            [96], num_channels)
        self.dc_stage3, pre_stage_channels = self._make_stage(
            self.RGB_stage3_cfg, num_channels)

        self.RGB_stage4_cfg = extra['STAGE4']
        num_channels = self.RGB_stage4_cfg['NUM_CHANNELS']
        block = blocks_dict[self.RGB_stage4_cfg['BLOCK']]
        num_channels = [
            num_channels[i] * block.expansion for i in range(len(num_channels))]
        self.dc_transition3 = self._make_transition_layer(
            pre_stage_channels, num_channels)
        self.dc_stage4, pre_stage_channels = self._make_stage(
            self.RGB_stage4_cfg, num_channels, multi_scale_output=True)
    ...
    def forward(self, x):
        # RGB branch
        # x = self.RGB_layer0_0_dil(x)
        # x = self.RGB_layer1(x)
        # x = self.RGB_layer1_tail(x)
        # B, C, H, W = x.shape
        # x = x.reshape(B, C, H//8, 8, W//8, 8).permute(0, 1, 3, 5, 2, 4).reshape(B, 64*C, H//8, W//8)
        x = self.RGB_dct(x)
        x = self.RGB_layer2(x)  # x.shape = torch.Size([1, 96, 64, 64])

        x_list = []
        for i in range(self.RGB_stage3_cfg['NUM_BRANCHES']):
        ...
        x_list = []
        for i in range(self.RGB_stage4_cfg['NUM_BRANCHES']):
